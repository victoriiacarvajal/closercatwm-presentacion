          ...
          <div className="grid gap-10 md:grid-cols-2 md:items-center">
            <div className="relative overflow-hidden rounded-2xl shadow-lg">
              <div className="flex transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${currentSlide * 100}%)` }}>
                {[
                  { src: '/closercat-dashboard.png', alt: 'Dashboard principal' },
                  { src: '/closercat-conversacion.png', alt: 'Vista de conversaciones' },
                  { src: '/closercat-contexto.png', alt: 'Contexto del lead' },
                  { src: '/closercat-campañas.png', alt: 'Gestión de campañas' },
                  { src: '/closercat-busqueda.png', alt: 'Búsqueda avanzada' },
                  { src: '/closercat-cierre.png', alt: 'Gestión de cierre' },
                  { src: '/closercat-multimedia.png', alt: 'Contenido multimedia' },
                  { src: '/closercat-plantillas.png', alt: 'Plantillas de respuesta' },
                  { src: '/closercat-integracion.png', alt: 'Integraciones' },
                ].map((slide, idx) => (
                  <div key={idx} className="w-full flex-shrink-0">
                    ...
                    <div className="p-3 text-center">
                      <div className="text-sm font-semibold text-gray-900">{slide.alt}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Navigation arrows */}
              <button
                onClick={() => setCurrentSlide((prev) => (prev === 0 ? 8 : prev - 1))}
                className="absolute left-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-gray-800 shadow-lg transition-colors hover:bg-white text-sm"
                aria-label="Anterior"
              >
                ‹
              </button>
              <button
                onClick={() => setCurrentSlide((prev) => (prev === 8 ? 0 : prev + 1))}
                className="absolute right-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-gray-800 shadow-lg transition-colors hover:bg-white text-sm"
