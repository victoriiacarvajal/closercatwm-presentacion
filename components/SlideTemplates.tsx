...
// 1. Cover Slide
export const CoverSlide: React.FC<TemplateProps> = ({ data }) => {
    ...
    <div className="h-full flex flex-col items-center justify-center text-center px-16 relative">
      <div className="mb-10">
          <img
            ...
            alt="CloserCat Logo"
            className="h-24 mx-auto object-contain drop-shadow-xl"
          />
      </div>
      <h2 className="text-3xl text-brand-cyan font-medium mb-16 max-w-4xl">{data.subtitle}</h2>
...
export const AgendaSlide: React.FC<TemplateProps> = ({ data }) => (
  <div className="h-full flex flex-col justify-center px-24">
    <h2 className="text-6xl font-display font-bold text-gray-900 mb-16 border-l-[12px] border-brand-purple pl-10">{data.title}</h2>
    <div className="space-y-8">
      {data.bullets?.map((item, i) => (
        <div key={i} className="flex items-center gap-8 group">
            <span className="font-display font-bold text-5xl text-gray-200 group-hover:text-brand-cyan transition-colors">
                {(i + 1).toString().padStart(2, '0')}
            </span>
            <div className="text-3xl font-medium text-gray-700 group-hover:text-gray-900 transition-colors">
...
export const TransitionSlide: React.FC<TemplateProps> = ({ data }) => (
  <div className="h-full flex flex-col items-center justify-center bg-gray-900 text-white relative overflow-hidden">
    ...
    <h1 className="text-7xl font-display font-bold mb-8 text-center z-10 px-12 leading-tight max-w-6xl">{data.title}</h1>
    {data.subtitle && <h2 className="text-4xl font-light text-brand-cyan z-10 text-center max-w-4xl">{data.subtitle}</h2>}
  </div>
);
...
export const StandardSlide: React.FC<TemplateProps> = ({ data }) => {
  ...
  return (
    <div className="h-full flex flex-col px-20 py-12 relative">
      <h2 className="text-5xl font-display font-bold text-gray-900 mb-4">{data.title}</h2>
      {data.subtitle && <h3 className="text-2xl text-brand-purple font-medium mb-12">{data.subtitle}</h3>}
      
      <div className="flex-1 flex gap-16 items-center">
          <div className="flex-1 space-y-8">
              {data.content && <p className="text-2xl text-gray-600 leading-relaxed mb-8">{data.content}</p>}
              {data.bullets?.map((bull, i) => (
                  <div key={i} className="flex gap-4">
                      <div className="w-2.5 h-2.5 rounded-full bg-brand-cyan mt-3.5 flex-shrink-0" />
                      <p className="text-2xl text-gray-700 leading-snug">{bull}</p>
                  </div>
              ))}
               {data.extraText && (
                  <div className="mt-10 p-8 bg-brand-cyan/10 border-l-8 border-brand-cyan rounded-r-xl">
                      <p className="font-bold text-brand-cyan text-xl">{data.extraText}</p>
                  </div>
              )}
          </div>
          ...
          {data.imagePlaceholder && (
               <div className="flex-1 self-stretch flex items-center justify-center pl-6">
                   <div 
                      className="w-full bg-gray-100 rounded-2xl flex items-center justify-center text-center p-6 relative overflow-hidden border border-gray-200 shadow-2xl aspect-square transform hover:scale-[1.01] transition-transform group"
                      onClick={handleOpenVideo}
                      style={!isLocalVideo && data.videoUrl ? { cursor: 'pointer' } : undefined}
                   >
                       {isLocalVideo ? (
...
export const SplitImageSlide: React.FC<TemplateProps> = ({ data }) => {
  ...
  return (
    <div className="h-full flex px-20 py-12 gap-16 relative">
        <div className="flex-1 flex flex-col justify-center">
            <h2 className="text-5xl font-display font-bold text-gray-900 mb-8 leading-tight">{data.title}</h2>
            {data.content && <p className="text-2xl text-gray-600 mb-10 leading-relaxed">{data.content}</p>}
            
            <div className="space-y-6">
                 {data.bullets?.map((bull, i) => (
                    <div key={i} className="flex gap-4 items-start">
                        <Check className="w-6 h-6 text-brand-green mt-1 flex-shrink-0" />
                        <p className="text-xl text-gray-700 font-medium leading-snug">{bull}</p>
                    </div>
                ))}
            </div>
            ...
            {data.extraText && (
                 <div className="mt-12 pt-8 border-t border-gray-200">
                    <p className="text-brand-purple font-semibold italic text-xl">{data.extraText}</p>
                </div>
            )}
        </div>
...
export const DashboardSlide: React.FC<TemplateProps> = ({ data }) => {
  ...
  return (
    <div className="h-full flex flex-col px-16 py-10 relative">
        <div className="mb-8 text-center">
             <h2 className="text-5xl font-display font-bold text-gray-900">{data.title}</h2>
             <p className="text-2xl text-brand-cyan mt-3">{data.subtitle}</p>
        </div>
        <div 
...
export const GridSlide: React.FC<TemplateProps> = ({ data }) => (
    <div className="h-full flex flex-col px-16 py-10">
        <h2 className="text-4xl font-display font-bold text-gray-900 mb-3">{data.title}</h2>
        {data.subtitle && <p className="text-xl text-gray-500 mb-10">{data.subtitle}</p>}
                ...
                <div key={i} className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100 hover:border-brand-cyan/50 transition-colors flex flex-col">
                    ...
                    <ul className="space-y-4 flex-1">
                        {col.content.map((item: string, j: number) => (
                            <li key={j} className="text-lg text-gray-600 flex gap-3 leading-snug">
                                <span className="text-brand-cyan text-xl">•</span> {item}
                            </li>
                        ))}
                    </ul>
                </div>
...
export const SplitTextSlide: React.FC<TemplateProps> = ({ data }) => (
     <div className="h-full flex flex-col px-16 py-10">
        <h2 className="text-4xl font-display font-bold text-center text-gray-900 mb-12">{data.title}</h2>
        
        <div className="flex-1 flex gap-12">
            {data.columns?.map((col, i) => (
                <div key={i} className={`flex-1 rounded-3xl p-10 border-2 shadow-sm ${i === 0 ? 'bg-brand-green/5 border-brand-green/30' : 'bg-gray-50 border-gray-200'}`}>
                    <h3 className="font-bold text-3xl mb-8 flex items-center gap-3">
                        {col.title}
                    </h3>
                    <ul className="space-y-6 mb-10">
                         {col.content.map((item: string, j: number) => (
                            <li key={j} className="flex gap-4 text-xl text-gray-700 leading-snug">
                    ...
                    <div className="h-40 bg-white rounded-xl border-2 border-dashed border-gray-300 flex items-center justify-center text-sm text-gray-400 font-mono text-center px-6 relative overflow-hidden group">
        ...
        {data.footerText && <p className="text-center font-bold text-2xl text-brand-purple mt-10">{data.footerText}</p>}
     </div>
);
...
export const ComparisonTableSlide: React.FC<TemplateProps> = ({ data }) => (
    <div className="h-full flex flex-col px-12 py-8">
        <h2 className="text-4xl font-display font-bold text-gray-900 mb-8">{data.title}</h2>
        
        <div className="overflow-hidden rounded-2xl border border-gray-200 shadow-md flex-1">
            <table className="w-full h-full text-base text-left">
                <thead className="bg-gray-50 text-gray-700 font-display text-lg">
                    <tr>
                        ...
                        <th className="p-4 bg-brand-cyan/10 text-brand-cyan font-bold border-b-4 border-brand-cyan text-center">CloserCat</th>
                        <th className="p-4 text-center">Chatbots Genéricos</th>
                        <th className="p-4 text-center">WhatsApp Manual</th>
                        <th className="p-4 text-center">CRMs + WA</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                    {[
                        ...
                        <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                            <td className="p-4 pl-6 font-medium text-gray-800 text-lg">{row[0]}</td>
                            <td className="p-4 bg-brand-cyan/5 font-bold text-brand-cyan text-center text-xl">{row[1]}</td>
                            <td className="p-4 text-center text-gray-500 text-lg">{row[2]}</td>
                            <td className="p-4 text-center text-gray-500 text-lg">{row[3]}</td>
                            <td className="p-4 text-center text-gray-500 text-lg">{row[4]}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
...
export const PricingSlide: React.FC<TemplateProps> = ({ data }) => (
    <div className="h-full flex flex-col px-10 py-8 bg-gray-50/50">
        <div className="text-center mb-8">
            <h2 className="text-4xl font-display font-bold text-gray-900">{data.title}</h2>
            <p className="text-gray-600 font-medium mt-2 text-xl">{data.subtitle}</p>
        </div>
        
        {/* Tiers Grid */}
        <div className="grid grid-cols-4 gap-6 flex-1 items-start mb-8">
            {data.columns?.map((plan, i) => {
                ...
                return (
                    <div key={i} className={`relative flex flex-col h-full bg-white rounded-2xl shadow-lg border-2 ${isPopular ? 'border-brand-purple scale-105 z-10 shadow-2xl' : 'border-transparent shadow-md'} transition-all overflow-hidden`}>
                        ...
                        <div className="p-6 border-b border-gray-100 text-center bg-gray-50/30">
                            <h3 className={`text-2xl font-extrabold ${isPopular ? 'text-brand-purple' : 'text-gray-800'}`}>{plan.title}</h3>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col">
                            ...
                            {hasMessage && (
                              <div className="p-5 bg-brand-cyan/5 border-b border-gray-100 flex flex-col items-center text-center">
                                  <div className="text-xs font-bold uppercase tracking-wider text-brand-cyan mb-2 flex items-center gap-1">
                                      <Bot size={12} /> Suscripción por mensajes
                                  </div>
                                  <div className="mb-2">
                                      <span className="text-3xl font-bold text-gray-900">{plan.priceMessage}</span>
                                      <span className="text-xs text-gray-500 font-bold ml-1">{plan.unitMessage}</span>
                                  </div>
                                  <p className="text-xs text-gray-600 font-medium px-2">{plan.detailMessage}</p>
                              </div>
                            )}

                            {/* Divider OR (only if both models exist) */}
                            {hasMessage && hasPlatform && (
                              <div className="relative h-6 bg-gray-100/50">
                                  <div className="absolute inset-0 flex items-center justify-center">
                                      <span className="bg-white border border-gray-200 text-[10px] font-bold text-gray-400 px-3 py-0.5 rounded-full uppercase shadow-sm">O</span>
                                  </div>
                              </div>
                            )}
                            ...
                            {hasPlatform && (
                              <div className="p-5 bg-brand-purple/5 border-b border-gray-100 flex flex-col items-center text-center">
                                  <div className="text-xs font-bold uppercase tracking-wider text-brand-purple mb-2 flex items-center gap-1">
                                      <User size={12} /> Opción B
                                  </div>
                                  <div className="mb-2">
                                      <span className="text-3xl font-bold text-gray-900">{plan.pricePlatform}</span>
                                      <span className="text-xs text-gray-500 font-bold ml-1">{plan.unitPlatform}</span>
                                  </div>
                                  <p className="text-xs text-gray-600 font-medium px-2">{plan.detailPlatform}</p>
                              </div>
                            )}

                            {/* Shared Features / Benefits */}
                            <div className="p-6 space-y-3 bg-white flex-1">
                                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 text-center">Beneficios incluidos al contratar por mensajes</p>
                                {plan.features.map((feat: string, j: number) => (
                                    <div key={j} className="text-sm text-gray-600 flex gap-3 items-start leading-tight">
                         ...
                         <div className="p-6 pt-0 mt-2">
                            <button className={`w-full py-3 rounded-xl text-base font-bold transition-colors shadow-sm ${isPopular ? 'bg-brand-purple text-white hover:bg-purple-800' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                                Elegir {plan.title}
                            </button>
                        </div>
                    </div>
        ...
        {/* Footer Info */}
        {data.footerText && (
          <div className="text-center text-base text-gray-600 font-medium space-y-1">
...
export const TimelineSlide: React.FC<TemplateProps> = ({ data }) => (
    <div className="h-full flex flex-col px-12 py-8">
        <h2 className="text-5xl font-display font-bold text-center text-gray-900 mb-10">{data.title}</h2>
        
        {/* Milestones Row */}
        <div className="flex gap-4 items-stretch justify-center mb-10">
            {data.columns?.map((col, i) => (
                <React.Fragment key={i}>
                    <div className="flex-1 bg-white p-6 rounded-xl shadow-md border-t-8 border-brand-cyan flex flex-col transform hover:-translate-y-1 transition-transform">
                        <h3 className="font-bold text-xl text-gray-800 mb-4 uppercase tracking-wide flex items-center gap-2">
                        ...
                        <ul className="space-y-3">
                             {col.content.map((item: string, j: number) => (
                                <li key={j} className="text-lg text-gray-600 flex gap-3 items-start leading-snug">
                ...
                <div className="space-y-4 flex-1 overflow-auto pr-2">
                    {data.addOns?.map((addon, i) => (
                        <div key={i} className="flex justify-between items-center border-b border-gray-200 pb-3 last:border-0 last:pb-0">
                            <div>
                                <p className="font-bold text-gray-800 text-lg">{addon.title}</p>
                                <p className="text-sm text-gray-500 font-medium">{addon.detail}</p>
                            </div>
                            <div className="text-right font-mono font-bold text-brand-cyan text-xl whitespace-nowrap ml-4">
                                {addon.price}
                            </div>
                        </div>
                    ))}
